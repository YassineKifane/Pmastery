...
export default function HomeScreen() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await axios.get(`${URL}/demande/hasDemand`, {
          params: { userId: userInfo.userId },
          headers: { Authorization: `${userInfo.token}` },
        });
        const demandData = response.data;
        setHasDemand(demandData);
  ...
  const requestStageSheet = async () => {
    try {
      const confirmation = window.confirm("Êtes-vous sûr de vouloir demander la fiche de stage ?");
      if (confirmation) {
        // Envoyer la demande de fiche de stage
        await axios.post(`${URL}/demande/create`, {
          studentId: userInfo.userId,
          affiliationCode: userInfo.affiliationCode
        });
        toast.success('Demande de fiche de stage envoyée avec succès.');
      }
    }  catch (error) {
      console.error(error);
        ...
        <Container fluid>
          <Row>
            <Col sm={12} md={7} className="mb-3 mb-md-0">
              ...
              {userInfo.role === 'STUDENT' &&  !hasDemand &&  hasPFE &&(
                  <Row className="mb-2">
                    <Button onClick={requestStageSheet}>Demander la fiche de stage</Button>
                  </Row>
              )}
